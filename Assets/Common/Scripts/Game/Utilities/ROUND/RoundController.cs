...
public class RoundController : Singlenton<RoundController>
{
    ...
    {
        bool aux = true;
        if (valueFicha <= cashTotal && valueFicha != 0)
        {
            aux = true;
    ...
    private void ResetButtons()
    {
        ...
        {
            ButtonDisplay sc = btn.GetComponent<ButtonDisplay>();
            if (sc.GetCurrentFichasOnTop() != 0) 
            {
                sc.ActivateButton(true);
                sc.ResetButton();
            }
        }
    }
    public void ActivateButtons(bool isOn)
    {
        foreach (var btn in _goButtons)
        {
            btn.GetComponent<ButtonDisplay>().ActivateButton(isOn);
        }
    }
